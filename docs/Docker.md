...
`wlandry/sdpb:2.1.1` containing binaries for `scalar_blocks`,
...
together on a single line
    ...
    docker run -v /my/project/:/usr/local/share/sdpb wlandry/sdpb:2.1.1 mpirun --allow-run-as-root -n 4 pvm2sdp 1024 /usr/local/share/sdpb/input.xml /usr/local/share/sdpb/input

Running this command will populate the directory `/my/project/input`.
To search for primal-dual solutions

    docker run -v /my/project/:/usr/local/share/sdpb wlandry/sdpb:2.1.1 mpirun --allow-run-as-root -n 4 sdpb --precision=1024 --procsPerNode=4 -s /usr/local/share/sdpb/input
