...
convert the image
    ...
    singularity pull docker://wlandry/sdpb:2.3.0

Depending on your version of Singularity, this will create a file
named `sdpb-2.3.0.simg` or `sdpb_2.3.0.sif`.

Singularity should automatically mount your home directory.  If your
Singularity image is named `sdpb-2.3.0.simg`, you can invoke the SDPB
programs by prepending the command with

    singularity exec sdpb-2.3.0.simg

So to convert the XML input file at `/home/user/input.xml`, run the command

    singularity exec sdpb-2.3.0.simg mpirun -n 4 pvm2sdp 1024 /home/user/input.xml /home/user/input
...
To find a primal-dual solution, 
    ...
    singularity exec sdpb-2.3.0.simg mpirun -n 4 sdpb --precision=1024 --procsPerNode=4 -s /home/user/input
