...
To improve performance, one may use a lower value (e.g. by specifying \texttt{"reducedPrefactor"}, \texttt{"maxNumPoles"} or the command-line option \texttt{pmp2sdp --maxNumPoles}, or choosing sample points explicitly, see below).
...
See \cite{Chang:2025mwt} and Section 3.3 in \cite{DSD} for details.
...
@\defn{positive matrix with prefactor $j$}@ {
	...
	@\textrm{\em{(optional)}}@ "maxNumPoles": @\repl{maximal number of poles in reducedPrefactor}@,
	"polynomials": [
...
\ee
Sample points $x_0 \ldots x_{d_j}$ are chosen to minimize interpolation errors on the positive real line \cite{Chang:2025mwt}.
...
See \cite{Chang:2025mwt} for detailed discussion of the interpolation algorithm.
...
If \texttt{"reducedPrefactor"} has more poles than specified by the field \texttt{"maxNumPoles"} or by the command-line option \texttt{pmp2sdp --maxNumPoles}, then only the rightmost \texttt{maxNumPoles} poles are kept in \texttt{"reducedPrefactor"}.
Negative \texttt{maxNumPoles} values are ignored.
...
%\cite{Chang:2025mwt}
\bibitem{Chang:2025mwt}
  C.~H.~Chang, V.~Dommes, P.~Kravchuk, D.~Poland and D.~Simmons-Duffin,
  ``Accurate bootstrap bounds from optimal interpolation,''
  [arXiv:2509.14307 [hep-th]].
%5 citations counted in INSPIRE as of 03 Apr 2026
