...
\begin{itemize}
	...
	\begin{itemize}
		\item polynomial matrices $W^0_j(x),\dots,W^N_j(x)$ of maximum degree $d_j$,
...
@\defn{positive matrix with prefactor $j$}@ {
	@\textrm{\em{(optional)}}@ "DampedRational": @\repl{damped-rational prefactor}@,
	"polynomials": [
	...
	@\textrm{\em{(optional)}}@ "bilinearBasis": [
...
If \texttt{"samplePoints"} are not specified, \texttt{pmp2sdp} will make a choice of sample points automatically.
For historical reasons, the choice is
\be
	x_k^{(j)} = \frac{\pi^2 (-1+4k)^2}{-64 (d_j + 1) \log(3-2\sqrt{2})}
.
\ee

If \texttt{"sampleScalings"} and/or \texttt{"bilinearBasis"} are not specified,
they are calculated automatically from the \texttt{"DampedRational"} prefactor.
Specifically, if the prefactor is $\chi(x)$, the bilinear basis is a set of orthogonal polynomials with respect to measure $\chi(x)dx$ on the positive real line, and sample scalings are $\chi(x_k)$, where the $x_k$ are sample points.
Only damped-rational prefactors are supported.
Such prefactors are relevant to the conformal bootstrap problems.  These stand for
\be
\texttt{DampedRational[$c$, \{$p_1,\dots,p_k$\}, $b$, $x$]} &\to& c\frac{b^x}{\prod_{i=1}^k (x-p_i)}.
\ee

If \texttt{"DampedRational"} is not specified, default sample scalings and bilinear basis are calculated assuming the prefactor $\chi(x) = e^{-x}$.
...
You can also provide a custom function \texttt{samplePointsFn[d]} generating sample points $x_0,\ldots,x_d$ for a polynomial of degree $d$.
...
\texttt{SDPB.m} also defines \texttt{WritePmpXml[file, sdp]} function which writes PMP in XML format described in Section \ref{sec:xml}.
In addition, \texttt{pmp2sdp} can also directly read .m files containing \texttt{SDP} definition.
...
@\defn{function call}@
WritePmpJson[file, @\repl{sdp}@]
@\textrm{\em or}@
WritePmpJson[file, @\repl{sdp}@, @\repl{samplePointsFn}@] 
...
@\defn{samplePointsFn[d\_]}@ function returning a list {$x_0$, ..., $x_d$}
\end{lstlisting}
...
Note that we recommend using \texttt{JSON} as more compact and efficient.
...
\ee
This is an PMP with $1\x1$ positive-semidefiniteness constraints.  We will arbitrarily choose a prefactor of $e^{-x}=\texttt{DampedRational[1,\{\}, 1/E,x]}$, so that the bilinear basis consists of Laguerre polynomials.  The \texttt{Mathematica} code for this example (see \texttt{mathematica/Tests.m}) is
...
Module[
  ...
  WritePmpJson["pmp.json", SDP[obj, norm, pols]];
];
\end{lstlisting}
...
as well as condition number for matrix $Q$, the worst condition number across all SDP blocks and the name of the corresponding block.
