    ...
    SECTION("write_profile")
    {
      ...
      DYNAMIC_SECTION("--maxIterations=" << max_iterations)
      {
        ...
        }
        {
          if(max_iterations == 1)
            {
              INFO("ck/block_timings is generated by timing run. "
                   "Actual run has only one iteration and shouldn't update "
                   "ck/block_timings.");
              REQUIRE(fs::file_size(runner.output_dir / "ck/block_timings")
                      > 0);
              REQUIRE(!fs::exists(runner.output_dir / "ck/block_timings.0"));
            }
          else
            {
              INFO("ck/block_timings.0 is generated by timing run, "
                   "ck/block_timings - by actual run.");
              REQUIRE(fs::file_size(runner.output_dir / "ck/block_timings")
                      > 0);
