...
function sdp2input_run_test() {
  ...
  TEST_RUN_SUCCESS "run-$filename" run_sdp2input "$filename" $args
  TEST_RUN_SUCCESS "check-result-for-$filename" DIFF_ZIP_IGNORE_CONTROL "$result" "$orig"
}
...
TEST_RUN_SUCCESS "profiling.0" CHECK_FILE_NOT_EMPTY "$output_dir/sdp2input_test.m/sdp.zip.profiling.0"
TEST_RUN_SUCCESS "profiling.1" CHECK_FILE_NOT_EMPTY "$output_dir/sdp2input_test.m/sdp.zip.profiling.1"
