...
/*void mpmat::trsm_reduced_gpu(
  ...
  ////std::cout << "The non-c++ code is running, I swear\n";
    ...
    timers["mpmat_trsm_reduced.complete"].resume();

    timers["mpmat_trsm_reduced.precalculations"].resume();
    ...
    timers["mpmat_trsm_reduced.precalculations"].stop();

    timers["mpmat_trsm_reduced.GMPtoDouble"].resume();
    ...
    timers["mpmat_trsm_reduced.GMPtoDouble"].stop();

    timers["mpmat_trsm_reduced.gpu_copy_forward"].resume();
    ...
    timers["mpmat_trsm_reduced.gpu_copy_forward"].stop();

    //cublasHandle_t handle;
    //cublasCreate(&handle);

    timers["mpmat_trsm_reduced.multiplication"].resume();
    ...
    for (int i = 0; i < mpmat_size_c; i++) {
        for (int j = 0; j <= i; j++) {
	  cublasDtrsm(handle, CUBLAS_SIDE_LEFT, "Lower",
		      transa == CblasTrans ? CUBLAS_OP_N : CUBLAS_OP_T,
		      diag,
		       m,
		       n,
    ...
    // cudaMemcpy(h_a,d_a,mem_a,cudaMemcpyDeviceToHost);
    //cudaMemcpy(h_b,d_b,mem_b,cudaMemcpyDeviceToHost);
    cudaMemcpy(c_double_array,d_c,mem_c*sizeof(mpmat_double),cudaMemcpyDeviceToHost);
