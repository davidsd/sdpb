...
void mpmat::karatsuba(const int & c_max, CBLAS_ORDER Layout, CBLAS_TRANSPOSE transa,
...
#ifdef __SDPB_CUDA__
void mpmat::karatsuba(const int & c_max, CBLAS_ORDER Layout, CBLAS_TRANSPOSE trans,
...
#ifdef __SDPB_CUDA__
// gemm karatsuba
void mpmat::karatsuba_cpu(const int & a_start, const int & b_start, const int & c_start, const int & c_max,
                      CBLAS_ORDER Layout, CBLAS_TRANSPOSE transa,
                      CBLAS_TRANSPOSE transb, const int & m, const int & n, const int & k,
                      const double alpha, const double beta){
  int start = a_start + b_start;
  ...
  if(diff <= 2){ // base case, just multiply. Currently set to do so on CPU: investigate switching to GPU in some case?
    cblas_dgemm(
            Layout,
            transa,
            transb,
  ...
  else if(mem < gpu_mem) { //if the problem can fit on the GPU in the next step, do so.

    #pragma omp parallel for
    for (int i = 0; i < 2; ++i){
      realloc_gpu_only(len2*m*k,len2*n*k,(6*len2+2)*m*n,i);
      clear_gpu(i);
      cudaMemcpy(d_a[i],a_double_array+k*m*(a_start+i*len2), len2*m*k*sizeof(mpmat_double),cudaMemcpyHostToDevice);
      cudaMemcpy(d_b[i],b_double_array+k*n*(b_start+i*len2), len2*n*k*sizeof(mpmat_double),cudaMemcpyHostToDevice);
    }
    // A_0 += A_1
    cblas_daxpy(k*m*len2, 1.0, a_double_array+k*m*(a_start+len2), 1, a_double_array+k*m*a_start, 1);
    ...
    #pragma omp parallel sections 
    {
      ...
      #pragma omp section
      {
        ...
        cudaMemcpy(d_a[2],a_double_array+k*m*(a_start), len2*m*k*sizeof(mpmat_double),cudaMemcpyHostToDevice);
        cudaMemcpy(d_b[2],b_double_array+k*n*(b_start), len2*n*k*sizeof(mpmat_double),cudaMemcpyHostToDevice);
    ...
    cudaSetDevice(1);
    cudaMemcpy(c_double_array+m*n*(c_start+2*clen2+1), d_c[1], (2*clen2+1)*m*n*sizeof(mpmat_double),cudaMemcpyDeviceToHost);
    ...
    // C_2 = A_1 * B_1 // conditional karatsuba, depending on the cutoff
    cudaSetDevice(2);
    cudaMemcpy(c_double_array+m*n*(c_start+3*clen2+2), d_c[2], (2*clen2+1)*m*n*sizeof(mpmat_double),cudaMemcpyDeviceToHost);
    ...
    // C_0 = A_0 * B_0 // full karatsuba
    karatsuba_cpu(a_start, b_start, c_start, 2*len2 + start, Layout, transa, transb, m, n, k);
    ...
    // C_1 = A_0 * B_0 // full karatsuba
    karatsuba_cpu(a_start, b_start, c_start+(2*clen2+1), 2*len2 + start, Layout, transa, transb, m, n, k);
    ...
    // C_2 = A_1 * B_1 // conditional karatsuba, depending on the cutoff
    if (diff <= 3*len2) // if we need half or less of C_2, then just use grade school
      gradeschool_cpu(a_start+len2, b_start+len2, c_start+(3*clen2+2), c_max, Layout, transa, transb, m, n, k);
    else               // otherwise, we need all quadrants and therefore karatsuba
      karatsuba_cpu(a_start+len2, b_start+len2, c_start+(3*clen2+2), c_max, Layout, transa, transb, m, n, k);
...
void mpmat::karatsuba_gpu(const int & a_start, const int & b_start, const int & c_start, const int & c_max,
  ...
  if(diff <= 2){ // base case, just multiply
    ...
    // A_0 += A_1
    cublasDaxpy(handles[device],k*m*len2, &alpha, d_a[device]+k*m*(a_start+len2), 1, d_a[device]+k*m*a_start, 1);
    
    // B_0 += B_1
    cublasDaxpy(handles[device],k*n*len2, &alpha, d_b[device]+k*n*(b_start+len2), 1, d_b[device]+k*n*b_start, 1);
    
    // C_1 = A_0 * B_0 // full karatsuba
    karatsuba_gpu(a_start, b_start, c_start+(2*clen2+1), 2*len2 + start, Layout, transa, transb, m, n, k, device);
    
    // A_0 -= A_1 // clean up
    cublasDaxpy(handles[device],k*m*len2, &minus, d_a[device]+k*m*(a_start+len2), 1, d_a[device]+k*m*a_start, 1);
    
    // B_0 -= B_1 // clean up
    cublasDaxpy(handles[device],k*n*len2, &minus, d_b[device]+k*n*(b_start+len2), 1, d_b[device]+k*n*b_start, 1);
    
    // C_1 -= C_0
    cublasDaxpy(handles[device],m*n*(2*clen2+1), &minus, d_c[device]+m*n*c_start, 1, d_c[device]+m*n*(c_start+(2*clen2+1)), 1);
    
    // C_02 += C_10
    cublasDaxpy(handles[device],m*n*clen2, &alpha, d_c[device]+m*n*(c_start+2*clen2+1), 1, d_c[device]+m*n*(c_start+clen2+1), 1);
    
    // squish the rest of C_1 against C_0
    cublasDcopy(handles[device],m*n*clen2, d_c[device]+m*n*(c_start+3*clen2+1), 1, d_c[device]+m*n*(c_start+2*clen2+1), 1);
    cublasDcopy(handles[device],m*n, d_c[device]+m*n*(c_start+4*clen2+1), 1, d_c[device]+m*n*(c_start+3*clen2+1), 1);

    // clear out the old
    cudaMemset(d_c[device]+m*n*(c_start+3*clen2+2),0,m*n*clen2*sizeof(double));
    
    // C_2 = A_1 * B_1 // conditional karatsuba, depending on the cutoff
    if (diff <= 3*len2) // if we need half or less of C_2, then just use grade school
      gradeschool_gpu(a_start+len2, b_start+len2, c_start+(3*clen2+2), c_max, Layout, transa, transb, m, n, k, device);
    else               // otherwise, we need all quadrants and therefore karatsuba
      karatsuba_gpu(a_start+len2, b_start+len2, c_start+(3*clen2+2), c_max, Layout, transa, transb, m, n, k, device);
    
    // C_1 -= C_2
    cublasDaxpy(handles[device],m*n*(2*clen2+1), &minus, d_c[device]+m*n*(c_start+(3*clen2+2)), 1, d_c[device]+m*n*(c_start+clen2+1), 1);
    
    // C_12 += C_20
    cublasDaxpy(handles[device],m*n*clen2, &alpha, d_c[device]+m*n*(c_start+3*clen2+2), 1, d_c[device]+m*n*(c_start+2*clen2+2), 1);
    
    // squish the rest
    cublasDcopy(handles[device],m*n*clen2, d_c[device]+m*n*(c_start+4*clen2+2), 1, d_c[device]+m*n*(c_start+3*clen2+2), 1);
    cublasDcopy(handles[device],m*n, d_c[device]+m*n*(c_start+5*clen2+2), 1, d_c[device]+m*n*(c_start+4*clen2+2), 1);
    
    cudaMemset(d_c[device]+m*n*(c_start+4*clen2+3),0,m*n*clen2*sizeof(double));
  }
...
void mpmat::karatsuba(const int & a_start, const int & c_start, const int & c_max,
                      CBLAS_ORDER Layout, CBLAS_TRANSPOSE trans, const int & n, const int & k,
                      const double alpha, const double beta){
...
#ifdef __SDPB_CUDA__
void mpmat::karatsuba_cpu(const int & a_start, const int & c_start, const int & c_max,
                      CBLAS_ORDER Layout, CBLAS_TRANSPOSE trans, const int & n, const int & k,
                      const double alpha, const double beta){
  ...
  if(diff <= 2){ // base case, just multiply
    ...
    for (int i = 0; i < 3; ++i){
      ...
      clear_gpu(i);
      cudaMemcpy(d_a[i],a_double_array+k*n*(a_start+i*len2), len2*n*k*sizeof(mpmat_double),cudaMemcpyHostToDevice);
    }
    // A_0 += A_1
    cblas_daxpy(k*n*len2, 1.0, a_double_array+k*n*(a_start+len2), 1, a_double_array+k*n*a_start, 1);
    ...
    #pragma omp parallel sections 
    {
      ...
      #pragma omp section
      {
        ...
        cudaMemcpy(d_a[2],a_double_array+k*n*(a_start), len2*n*k*sizeof(mpmat_double),cudaMemcpyHostToDevice);
    ...
    // C_0 = A_0 * A_0 // full karatsuba
    cudaSetDevice(0);
    cudaMemcpy(c_double_array+n*n*c_start, d_c[0], (2*clen2+1)*n*n*sizeof(mpmat_double),cudaMemcpyDeviceToHost);
    // C_1 = A_0 * A_0 // full karatsuba
    cudaSetDevice(1);
    cudaMemcpy(c_double_array+n*n*(c_start+2*clen2+1), d_c[1], (2*clen2+1)*n*n*sizeof(mpmat_double),cudaMemcpyDeviceToHost);
    ...
    // C_2 = A_1 * A_1 // conditional karatsuba, depending on the cutoff
    cudaSetDevice(2);
    cudaMemcpy(c_double_array+n*n*(c_start+3*clen2+2), d_c[2], (2*clen2+1)*n*n*sizeof(mpmat_double),cudaMemcpyDeviceToHost);
    ...
    // C_1 = A_0 * A_0 // full karatsuba
    karatsuba_cpu(a_start, c_start+(2*clen2+1), 2*len2 + start, Layout, trans, n, k);
    ...
    // C_2 = A_1 * A_1 // conditional karatsuba, depending on the cutoff
    if (diff <= 3*len2) // if we need half or less of C_2, then just use grade school
      gradeschool_cpu(a_start+len2, c_start+(3*clen2+2), c_max, Layout, trans, n, k);
    else               // otherwise, we need all quadrants and therefore karatsuba
      karatsuba_cpu(a_start+len2, c_start+(3*clen2+2), c_max, Layout, trans, n, k);
...
void mpmat::karatsuba_gpu(const int & a_start, const int & c_start, const int & c_max,
                      CBLAS_ORDER Layout, CBLAS_TRANSPOSE trans, const int & n, const int & k, int device,
                      const double alpha, const double beta){
  // std::cerr << "karatsuba on the " << device << " gpu.\n";
  //std::cerr << "skaratsuba " << a_start << "," << c_start << "," << c_max << "," << n << "," << k << "\n";
  int start = 2*a_start;
  int diff = c_max - start;
  cudaSetDevice(device);
  if(diff <= 2){ // base case, just multiply
    cublasDsyrk(handles[device],
                ...
                n,k,&alpha,
                d_a[device]+k*n*a_start,
                (Layout == CblasRowMajor) != (trans == CblasTrans) ? n : k,
                &beta,
                d_c[device]+n*n*c_start,
                n);
    // if (stat == CUBLAS_STATUS_SUCCESS) std::cerr << "multiplication succeeded\n";
    // else if (stat == CUBLAS_STATUS_NOT_INITIALIZED) std::cerr << "cublas wasn't set up\n";
    // else std::cerr << stat << "\n";
    ...
    // C_0 = A_0 * A_0 // full karatsuba
    karatsuba_gpu(a_start, c_start, 2*len2 + start, Layout, trans, n, k, device);
    
    // A_0 += A_1
    cublasDaxpy(handles[device],k*n*len2, &alpha, d_a[device]+k*n*(a_start+len2), 1, d_a[device]+k*n*a_start, 1);
    
    // C_1 = A_0 * A_0 // full karatsuba
    karatsuba_gpu(a_start, c_start+(2*clen2+1), 2*len2 + start, Layout, trans, n, k, device);
    
    // A_0 -= A_1 // clean up
    cublasDaxpy(handles[device],k*n*len2, &minus, d_a[device]+k*n*(a_start+len2), 1, d_a[device]+k*n*a_start, 1);
    
    // C_1 -= C_0
    cublasDaxpy(handles[device],n*n*(2*clen2+1), &minus, d_c[device]+n*n*c_start, 1, d_c[device]+n*n*(c_start+(2*clen2+1)), 1);
    
    // C_02 += C_10
    cublasDaxpy(handles[device],n*n*clen2, &alpha, d_c[device]+n*n*(c_start+2*clen2+1), 1, d_c[device]+n*n*(c_start+clen2+1), 1);
    
    // squish the rest of C_1 against C_0
    cublasDcopy(handles[device],n*n*clen2, d_c[device]+n*n*(c_start+3*clen2+1), 1, d_c[device]+n*n*(c_start+2*clen2+1), 1);
    cublasDcopy(handles[device],n*n, d_c[device]+n*n*(c_start+4*clen2+1), 1, d_c[device]+n*n*(c_start+3*clen2+1), 1);

    // clear out the old
    cudaMemset(d_c[device]+n*n*(c_start+3*clen2+2),0,n*n*clen2*sizeof(double));
    
    // C_2 = A_1 * A_1 // conditional karatsuba, depending on the cutoff
    if (diff <= 3*len2) // if we need half or less of C_2, then just use grade school
      gradeschool_gpu(a_start+len2, c_start+(3*clen2+2), c_max, Layout, trans, n, k, device);
    else               // otherwise, we need all quadrants and therefore karatsuba
      karatsuba_gpu(a_start+len2, c_start+(3*clen2+2), c_max, Layout, trans, n, k, device);
    
    // C_1 -= C_2
    cublasDaxpy(handles[device],n*n*(2*clen2+1), &minus, d_c[device]+n*n*(c_start+(3*clen2+2)), 1, d_c[device]+n*n*(c_start+clen2+1), 1);
    
    // C_12 += C_20
    cublasDaxpy(handles[device],n*n*clen2, &alpha, d_c[device]+n*n*(c_start+3*clen2+2), 1, d_c[device]+n*n*(c_start+2*clen2+2), 1);
    
    // squish the rest
    cublasDcopy(handles[device],n*n*clen2, d_c[device]+n*n*(c_start+4*clen2+2), 1, d_c[device]+n*n*(c_start+3*clen2+2), 1);
    cublasDcopy(handles[device],n*n, d_c[device]+n*n*(c_start+5*clen2+2), 1, d_c[device]+n*n*(c_start+4*clen2+2), 1);
    
    cudaMemset(d_c[device]+n*n*(c_start+4*clen2+3),0,n*n*clen2*sizeof(double));
  }
...
#ifdef __SDPB_CUDA__
// gemm gradeschool
void mpmat::gradeschool_cpu(const int & a_start, const int & b_start, const int & c_start, const int & c_max,
                      CBLAS_ORDER Layout, CBLAS_TRANSPOSE transa,
                      CBLAS_TRANSPOSE transb, const int & m, const int & n, const int & k,
                      const double alpha, const double beta){
  int start = a_start + b_start;
  ...
  if(diff < 2){ // base case, just multiply
    ...
    for (int i = 0; i < 3; ++i){
      ...
      clear_gpu(i);
      cudaSetDevice(i);
      cudaMemcpy(d_a[i],a_double_array+k*m*(a_start+(i % 2)*len2), len2*m*k*sizeof(mpmat_double),cudaMemcpyHostToDevice);
      cudaMemcpy(d_b[i],b_double_array+k*n*(b_start+(i / 2)*len2), len2*n*k*sizeof(mpmat_double),cudaMemcpyHostToDevice);
    }
    // for (int i = 0; i < 3; ++i){
    //   realloc_gpu_only(len2*m*k,len2*n*k,(6*len2+2)*m*n,0);
    //   cudaSetDevice(0);
    //   cudaMemcpy(d_a[0],a_double_array+k*m*(a_start+(i % 2)*len2), len2*m*k*sizeof(mpmat_double),cudaMemcpyHostToDevice);
    //   cudaMemcpy(d_b[0],b_double_array+k*n*(b_start+(i / 2)*len2), len2*n*k*sizeof(mpmat_double),cudaMemcpyHostToDevice);
    // }
    #pragma omp parallel sections 
    {
      #pragma omp section
    ...
    // C_0 = A_0 * B_0 // karatsuba
    karatsuba_cpu(a_start, b_start, c_start, c_max, Layout, transa, transb, m, n, k);
    
    // C_1 = A_1 * B_0 // grade school
    gradeschool_cpu(a_start+len2, b_start, c_start+(3*clen2+2), c_max, Layout, transa, transb, m, n, k);
    ...
    // C_2 = A_0 * B_1 // grade school // stored temporarily in C_2!
    gradeschool_cpu(a_start, b_start+len2, c_start+(3*clen2+2), c_max, Layout, transa, transb, m, n, k);
...
void mpmat::gradeschool_gpu(const int & a_start, const int & b_start, const int & c_start, const int & c_max,
  ...
  if(diff < 2){ // base case, just multiply
    ...
    int clen3 = len3/3; // the "fundamental length" of c one level below (pre-squishing)
    // C_0 = A_0 * B_0 // karatsuba
    karatsuba_gpu(a_start, b_start, c_start, c_max, Layout, transa, transb, m, n, k, device);
    
    //C_1 = A_1 * B_0 // grade school
    gradeschool_gpu(a_start+len2, b_start, c_start+(3*clen2+2), c_max, Layout, transa, transb, m, n, k, device);

    // move over C_1, deal with the overlap
    //std::cerr << "adding c[" << c_start+(3*clen2+2) << ":" <<  c_start+(3*clen2+2) + (2*clen2+1) << "] to c[" << (c_start+clen2+1) << ":" << (c_start+clen2+1) + (2*clen2+1) << "]\n";
    cublasDaxpy(handles[device],m*n*(2*clen2+1), &alpha, d_c[device]+m*n*(c_start+(3*clen2+2)), 1, d_c[device]+m*n*(c_start+clen2+1), 1);
    
    // clean up
    //std::cerr << "clearing c[" << (c_start+(3*clen2+2)) << ":" << (c_start+(3*clen2+2)) + (2*clen2+1) << "]\n";
    cudaMemset(d_c[device]+m*n*(c_start+(3*clen2+2)),0,m*n*(2*clen2+1)*sizeof(double));

    // C_2 = A_0 * B_1 // grade school // stored temporarily in C_2!
    gradeschool_gpu(a_start, b_start+len2, c_start+(3*clen2+2), c_max, Layout, transa, transb, m, n, k, device);
...
#endif
      n,n,1.0,c_double_array+n*n*(c_start+(3*clen2+2)+i),n,n);
    //std::cerr << "TRANSPOSED\n";
...
// syrk gradeschool
// IMPORTANT STEP: please make sure that a_double_array = b_double_array ahead of time
void mpmat::gradeschool_cpu(const int & a_start, const int & c_start, const int & c_max,
                      CBLAS_ORDER Layout, CBLAS_TRANSPOSE trans, const int & n, const int & k,
                      const double alpha, const double beta){ 
  int start = 2*a_start;
  int diff = c_max - start;
  ...
  int clen2 = len2 - 1; // the "fundamental length" of c one level below (post squishing)
  int clen3 = len3/3; // the "fundamental length" of c one level below (pre-squishing)
  size_t mem = ( 2*len2*n*k + (6*len2+2)*n*n)*sizeof(mpmat_double);
  // std::cerr << "sgradeschool (n,k)=(" << n << "," << k << ") " << start << " " << c_start << " " << c_max << "\n";
  if(diff < 2){ // base case, just multiply
    //std::cerr << "basecase'd out\n";
    cblas_dsyrk(CblasColMajor,
    ...
    clear_gpu(3);
    cudaMemcpy(d_a[3],a_double_array+k*n*(a_start), len2*n*k*sizeof(mpmat_double),cudaMemcpyHostToDevice);
    omp_set_dynamic(1);
    omp_set_nested(1);
    #pragma omp parallel sections 
    {
  ...
  //REVIEW
   ...
   int len2 = pow(2,ceil(log2(diff))-1)+.1;
    ...
    for (int i = 0; i < 2*clen2+1; ++i) 
      cublasDgeam(handles[device],CUBLAS_OP_T,CUBLAS_OP_N,n,n,&alpha,d_c[device]+n*n*(c_start+i),n, &beta, d_c[device]+n*n*(c_start+(3*clen2+2)+i),n, d_c[device]+n*n*(c_start+(3*clen2+2)+i),n);

    // move over C_0, deal with the overlap
    cublasDaxpy(handles[device],n*n*(2*clen2+1), &alpha, d_c[device]+n*n*(c_start), 1, d_c[device]+n*n*(c_start+3*clen2+2), 1);

    cudaMemset(d_c[device]+n*n*(c_start),0,n*n*(2*clen2+1)*sizeof(double));
    
    // C_0 = A_0 * A_0 // karatsuba
    karatsuba_gpu(a_start, c_start, c_max, Layout, trans, n, k);

    // move over C_1, deal with the overlap
    cublasDaxpy(handles[device],n*n*(2*clen2+1), &alpha, d_c[device]+n*n*(c_start+(3*clen2+2)), 1, d_c[device]+n*n*(c_start+clen2+1), 1);

    // clean up
    cudaMemset(d_c[device]+n*n*(c_start+(3*clen2+2)),0,n*n*(2*clen2+1)*sizeof(double));
  }
}
#endif
