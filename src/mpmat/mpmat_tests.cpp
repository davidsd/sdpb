...
bool compareSymmMatrices(double * x, double * y, int n, int l, bool lower, bool rowmaj){
  if (lower != !rowmaj){
      ...
      for (int j = 0; j < n; ++j){
        for (int k = 0; k <= j; ++k){
          if (x[n*n*i+n*j+k] != y[n*n*i+n*j+k]){
            std::cerr << "Error: matrices disagree at (" << i << "," << j << "," << k << ").\n";
            return false;
          }
        }
      }
    ...
    for (int i = 0; i < l; ++i){
      for (int j = 0; j < n; ++j){
        for (int k = 0; k <= j; ++k){
          if (x[n*n*i+n*(n-j-1)+(n-k)] != y[n*n*i+n*(n-j-1)+(n-k)]){
            std::cerr << "Error: matrices disagree at (" << i << "," << j << "," << k << ").\n";
            return false;
          }
        }
...
void mpmat::mpmat_conversion_test(int i, int f, int d){
  ...
  int mpmat_size = ceil_div( abs(in.get_mpf_t()->_mp_prec+1) * mp_bits_per_limb, mpmat_limb );
...
bool mpmat::karatsuba_test(int m, int n, int k, int l){
  ...
  //std::cout << a_double_array[0] << " times " << b_double_array[0] << " equals " << c2_double_array[0] << ".\n";

  // gradeschool_bc(0, 0, 0, l, CblasRowMajor, CblasNoTrans, CblasNoTrans, m, n, k);
  ...
  //std::cout << "condensing a tree with initial length: " << (int)pow(3,ceil(log2(l))) << "\n";
 ...
 // std::cout << "\n";
  ...
  if (!std::equal(c2_double_array,c2_double_array+n*m*l,c_double_array)){
...
bool mpmat::symm_karatsuba_test(int n, int k, int l){
  std::cerr << "estimated memory usage is " << n*n*(7*l - log2(l) - 1)*sizeof(double) / (1 << 20) << "\n";
  ...
  cblas_dgemm(CblasRowMajor,CblasNoTrans,
        ...
        k,
        a_double_array+(i-j)*k*n,
        k,
        1.0,
        c2_double_array+i*n*n,
        n);
        }
#ifdef HAVE_MKL_H
      mkl_domatcopy('c','t',
      ...
      cblas_domatcopy(CblasColMajor,CblasTrans,
        ...
        c2_double_array+i*n*n,
        n,
        b2_double_array+i*n*n,
        n
        );
      cblas_daxpy(n*n,1.0,
      b2_double_array+i*n*n,1,
      c2_double_array+i*n*n,1);
      // if significance of result is even, calculate the symmetric part
  if ( i % 2 == 0)
    cblas_dsyrk(CblasColMajor,CblasUpper,CblasTrans,
          n,k,1.0,a_double_array+k*n*(i/2),k,
          1.0,c2_double_array+i*n*n,n);
    }
 ...
 // std::cout << "\n";
  ...
  if (!std::equal(a_double_array,a_double_array+n*k*l,a2_double_array)){
    std::cerr << "a_double_array was modified\n";
    result = false;
    for (int i = 0; i < n*k*l; ++i)
      std::cout << a_double_array[i] << " ";
  ...
  if (!compareSymmMatrices(c_double_array,c2_double_array,n,l)){//(!std::equal(c2_double_array,c2_double_array+n*n*l,c_double_array)){
    std::cerr << "c_double_array is incorrect\n";
    result = false;
    for (int i = 0; i < n*n*(3*l - (int)log2(l) - 1); ++i)
      std::cout << c_double_array[i] << (i % (n*n) == n*n-1 ? "|" : (i % n == n-1 ? "," : " "));
    std::cout << "\n";
    for (int i = 0; i < n*n*l; ++i)
      std::cout << c2_double_array[i] << (i % (n*n) == n*n-1 ? "|" : (i % n == n-1 ? "," : " "));
    std::cout << "\n";
    //print_mpmat_double_array(c_double_array,m*n*(int)pow(3,ceil(log2(l))));
    //print_mpmat_double_array(c2_double_array,m*n*l);
  }
...
bool mpmat::symm_karatsuba_test_gpu(int n, int k, int l){
  ...
  // std::cout << "\n";
  memset(c2_double_array,0,n*n*l*sizeof(double));
  memset(c_double_array,0,n*n*(3*l - (int)log2(l) - 2)*sizeof(double));
  timers[std::string("setup.l=")+std::to_string(n)].stop();
...
//   double * tmp_a = a_double_array, * tmp_b = b_double_array, * tmp_c = c_double_array;
//   a_double_array = new double [1];
//   a_double_array[0] = 2.0;
...
//   std::cout << a_double_array[0] << " times " << b_double_array[0] << " equals " << c_double_array[0] << ".\n";
