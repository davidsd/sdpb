...
const BigInt_Shared_Memory_Syrk_Context::Job_Schedule &
                                                         ...
                                                         El::Int output_width)
{
  ...
  auto [it, res] = blas_job_schedule_cache.emplace(
    key, create_blas_job_schedule_func(kind, uplo, shared_memory_comm.Size(),
                                       comb.num_primes, output_height,
                                       output_width, verbosity));
  ASSERT(res);
  return it->second;
}