               ...
               const std::optional<El::UpperOrLower> &uplo = std::nullopt)
{
  ...
  if(input.DistComm().Size() == output_comm.Size())
    {
      for(El::Int iLoc = 0; iLoc < input.LocalHeight(); ++iLoc)
        for(El::Int jLoc = 0; jLoc < input.LocalWidth(); ++jLoc)
          {
            const auto i = input.GlobalRow(iLoc);
            ASSERT_EQUAL(i, output.GlobalRow(iLoc));

            const auto j = input.GlobalCol(jLoc);
            ASSERT_EQUAL(j, output.GlobalCol(jLoc));

            if(skip_element(i, j))
              continue;
            output.SetLocal(iLoc, jLoc, input.GetLocal(iLoc, jLoc));
          }
      return;
    }
