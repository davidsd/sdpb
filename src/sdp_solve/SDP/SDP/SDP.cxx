...
void SDP::validate(const Block_Info &block_info) const noexcept(false)
{
  ...
  // NB: normalization can be missing if SDP is from XML input,
  // or if SDP is generated by SDPB 2.6.1 or earlier.
  if(normalization.has_value())
    ASSERT_EQUAL(normalization->size(), dual_objective_b.Height() + 1);
