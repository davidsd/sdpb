                 ...
                 const El::BigFloat &threshold, const bool &need_lambda)
{
  ...
  for(size_t local_block_index = 0; local_block_index < pmp.blocks.size(); ++local_block_index)
    {
      ...
      const auto zero_values = find_zeros(c_minus_By_block, pvm_info, threshold);
      if(need_lambda)
        {
          ASSERT(x.has_value());
          compute_lambda(pvm_info.sample_points, pvm_info.reduced_sample_scalings,
                         pvm_info.preconditioning_vector, pvm_info.reduced_prefactor,
                         pvm_info.dim, x->at(local_block_index), zero_values,
                         pvm_info.block_index, spectrum_block.zeros,
