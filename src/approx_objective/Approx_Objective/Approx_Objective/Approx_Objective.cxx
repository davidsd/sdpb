...
// I don't know how to take trace properly
El::BigFloat matrix_trace(const Block_Diagonal_Matrix &A)
{
	Block_Diagonal_Matrix Id(A);
	Id.set_zero();
	Id.add_diagonal(1);
	return frobenius_product_symmetric(A, Id);
}
	...
	const El::DistMatrix<El::BigFloat> &Q)
{
	...
	Block_Diagonal_Matrix dX(X_cholesky);
	constraint_matrix_weighted_sum(block_info, sdp, dx, dX);

	Block_Diagonal_Matrix &X_inv_dX = dX;
	cholesky_solve(X_cholesky, X_inv_dX);

	Lag_pu = -matrix_trace(X_inv_dX);
}
