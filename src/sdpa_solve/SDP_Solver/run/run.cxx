...
namespace Sdpb::Sdpa
{
  ...
  namespace
  {
                                                 ...
                                                 const Verbosity verbosity)
    {
      ...
      // #Q = MxM, distributed over all nodes.
      const size_t S_size
        = El::mpi::Reduce(get_S_size_local(sdp), 0, node_comm);
      ...
      // X, Y, X_cholesky, Y_cholesky, primal_residues, dX, dY, XY, R, Z
      mem_required_size += 10 * X_size;

      // S = MxM
      mem_required_size += S_size;
    ...
    get_max_shared_memory_bytes(const size_t default_max_shared_memory_bytes,
                                const Environment &env, const SDP &sdp,
                                const SDP_Solver &solver,
                                const Verbosity verbosity)
    {
      ...
      const size_t nonshared_memory_required_per_node_bytes
        = get_required_nonshared_memory_per_node_bytes(env, sdp, solver,
                                                       verbosity);
    ...
    El::Matrix<int32_t> &block_timings_ms)
  {
    ...
    auto max_shared_memory_bytes = get_max_shared_memory_bytes(
      parameters.max_shared_memory_bytes, env, sdp, *this, verbosity);
