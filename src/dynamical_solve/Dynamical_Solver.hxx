...
public:
  ...
  bool
    lowest_mu_Q; // true : the solver hasn't been lifted along the local central path
  ...
  run_dynamical(const Environment &env,
  ...
  void external_corrector_step(
    const Dynamical_Solver_Parameters &dynamical_parameters,
    const SDP &old_sdp, const SDP &new_sdp, const Block_Info &block_info,
    const El::Grid &grid, const std::size_t &total_psd_rows,
  ...
  void compute_corrector_residue_shift(
  ...
  save_solver_state(const Dynamical_Solver_Parameters &dynamical_parameters,
                    const Block_Info &block_info, Block_Vector &dx,
                    Block_Vector &dy, Block_Diagonal_Matrix &dX,
                    Block_Diagonal_Matrix &dY,
                    const Block_Diagonal_Matrix &schur_complement_cholesky,
                    const Block_Matrix &schur_off_diagonal,
  ...
  void
  save_checkpoint(Block_Vector &dx, Block_Vector &dy,
  ...
  void
  internal_step(const Dynamical_Solver_Parameters &dynamical_parameters,
  ...
  void
  execute_step(Block_Vector &dx, Block_Vector &dy, Block_Diagonal_Matrix &dX,
               Block_Diagonal_Matrix &dY, El::BigFloat &primal_step_length,
               El::BigFloat &dual_step_length);

  void
  compute_dXdY(const bool &is_primal_and_dual_feasible,
               const Block_Info &block_info, const SDP &sdp,
               const El::Grid &grid, const Block_Diagonal_Matrix &X_cholesky,
               const Block_Diagonal_Matrix &Y_cholesky, Timers &timers,
  ...
  void strategy_update_grad_BFGS(El::BigFloat &primal_step_length,
                                 El::BigFloat &dual_step_length,
  ...
  El::BigFloat
  finite_mu_navigator(const Block_Info &block_info,
                      const Block_Diagonal_Matrix &X_cholesky,
                      const std::size_t &total_psd_rows, const int dim_y,
                      const El::BigFloat &mu, const El::BigFloat &beta,
                      const Dynamical_Solver_Parameters &dynamical_parameters);

  void dynamical_step_scan_beta(
    const Dynamical_Solver_Parameters &dynamical_parameters,
    ...
    int n_external_parameters, El::Matrix<El::BigFloat> &prev_grad,
    El::Matrix<El::BigFloat> &prev_step, El::Matrix<El::BigFloat> &prev_BFGS,
    El::Matrix<El::BigFloat> &external_step, Block_Diagonal_Matrix &dX_best,
    Block_Diagonal_Matrix &dY_best, Block_Vector &dx_best,
    Block_Vector &dy_best, El::BigFloat &primal_step_length_best,
    El::BigFloat &dual_step_length_best, El::BigFloat &beta_best,
    El::Matrix<El::BigFloat> &hess_BFGS_best,
    El::Matrix<El::BigFloat> &grad_mixed_best, bool &update_sdp,
