...
void Dynamical_Solver::external_corrector_step(
  const Dynamical_Solver_Parameters &dynamical_parameters, const SDP &old_sdp,
  const SDP &new_sdp, const Block_Info &block_info, const El::Grid &grid,
  const std::size_t &total_psd_rows, Block_Diagonal_Matrix &X_cholesky,
  ...
  const Block_Diagonal_Matrix &schur_complement_cholesky,
  const Block_Matrix &schur_off_diagonal,
  const El::DistMatrix<El::BigFloat> &Q, El::BigFloat &current_error_R,
  El::BigFloat &current_mu, El::BigFloat &primal_step_length,
  El::BigFloat &dual_step_length, El::BigFloat &step_length_reduction,
  Timers &timers)
{
  Block_Diagonal_Matrix primal_residues_0(X);
  Block_Vector dual_residues_0(block_info.schur_block_sizes(),
  ...
  compute_corrector_R(block_info, total_psd_rows, x, dx, y, dy, X, dX, Y, dY,
                      1, 1, R_0, R_error_0, coit_mu, timers);

  scale_multiply_add(El::BigFloat(-1), X, Y, El::BigFloat(0), R_0);
  R_0.add_diagonal(coit_mu);

  Block_Diagonal_Matrix R_old(X);
  El::BigFloat R_err_old, coit_mu_old;
  compute_R_error(total_psd_rows, X, Y, R_old, R_err_old, coit_mu_old, timers);

  if(El::mpi::Rank() == 0)
    std::cout << "before external correcter step, R_err(X)=" << R_err_old
              << " mu=" << coit_mu_old << " R_err(X+dX)=" << R_error_0
              << " mu=" << coit_mu << "\n"
              << std::flush;
  ...
  compute_corrector_residue_shift(block_info, primal_residue_p_0,
                                  dual_residues_0, R_0, primal_residue_p,
                                  dual_residues, R, dx, dy, dX, dY, d_sdp);
  ...
  compute_dXdY(true, block_info, old_sdp, grid, X_cholesky, Y_cholesky, timers,
               dx, dy, dX, dY, R, primal_step_length, dual_step_length,
               step_length_reduction);

  ///////// test ///////////////
  El::BigFloat error_P, error_p, error_d, error_R;
  compute_corrector_errors(block_info, new_sdp, total_psd_rows, x, dx, y, dy,
                           X, dX, Y, dY, 1, 1, error_P, error_p, error_d,
                           error_R, coit_mu, timers);

  if(El::mpi::Rank() == 0)
    std::cout << "external corrector : "
              << "R=" << error_R << " mu=" << coit_mu << " step-len : ("
...
void read_Schur_data(std::filesystem::path schur_path,
                     const Block_Info &block_info,
                     Block_Diagonal_Matrix &schur_complement_cholesky,
                     Block_Matrix &schur_off_diagonal,
                     El::DistMatrix<El::BigFloat> &Q)
{
  if(std::filesystem::exists(schur_path / "Q_cholesky.txt"))
    {
      for(size_t block = 0; block != block_info.block_indices.size(); ++block)
        {
          ...
          read_text_block(schur_complement_cholesky.blocks.at(block),
                          ...
                          block_index);
          read_text_block(schur_off_diagonal.blocks.at(block), schur_path,
...
void Dynamical_Solver::external_corrector_run(
  const Dynamical_Solver_Parameters &dynamical_parameters, const SDP &new_sdp,
  const Block_Info &block_info, const El::Grid &grid, Block_Vector &dx,
  ...
  El::BigFloat &step_length_reduction, Timers &timers)
{
  ...
  SDP old_sdp(dynamical_parameters.old_sdp_path, block_info, grid, timers);
  ...
  load_binary_checkpoint(dynamical_parameters.old_schur_path / (".ext_ck"), dx,
                         dy, dX, dY, verbosity_save);

  Block_Vector dx_last(dx);
  Block_Vector dy_last(dy);
  Block_Diagonal_Matrix dX_last(dX);
  Block_Diagonal_Matrix dY_last(dY);
  El::BigFloat error_R_last, mu_last, current_error_R, current_mu,
    primal_step_length_last, dual_step_length_last;
  ...
  for(int i = 0; i < 5; i++)
    {
      external_corrector_step(dynamical_parameters, old_sdp, new_sdp,
                              block_info, grid, total_psd_rows, X_cholesky,
                              Y_cholesky, A_X_inv, A_Y, dx, dy, dX, dY,
                              schur_complement_cholesky, schur_off_diagonal, Q,
                              current_error_R, current_mu, primal_step_length,
                              dual_step_length, step_length_reduction, timers);

      if(El::mpi::Rank() == 0)
        std::cout << "external corrector : R_error=" << current_error_R
                  << " mu=" << current_mu << "\n"
                  << std::flush;

      if(current_error_R > error_R_last)
        break;
...
void compute_By(const Block_Info &block_info, const SDP &sdp,
                const Block_Vector &y, Block_Vector &By)
{
  ...
  for(auto &block_index : block_info.block_indices)
    {
      ...
      Gemm(El::Orientation::NORMAL, El::Orientation::NORMAL, El::BigFloat(1),
           *free_var_matrix_block, *y_block, El::BigFloat(1), *By_block);
...
void compute_db_minus_dBnewx(const Block_Info &block_info, const SDP &d_sdp,
  ...
  for(auto &block_index : block_info.block_indices)
    {
      El::Gemv(El::OrientationNS::TRANSPOSE, El::BigFloat(-1),
               *free_var_matrix_block, *x_block, El::BigFloat(0), *rslt_block);
...
void Dynamical_Solver::compute_corrector_residue_shift(
  const Block_Info &block_info, Block_Vector &primal_residue_p_0,
  Block_Vector &dual_residues_0, Block_Diagonal_Matrix &R_0,
  Block_Vector &primal_residue_p, Block_Vector &dual_residues,
  Block_Diagonal_Matrix &R, Block_Vector &dx, Block_Vector &dy,
  Block_Diagonal_Matrix &dX, Block_Diagonal_Matrix &dY, const SDP &d_sdp)
{
