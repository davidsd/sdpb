	...
	bool &update_sdp, bool &find_zeros, El::Matrix<El::BigFloat> &external_step)
{
	...
	for (int stallrecovery_phase = 1; stallrecovery_phase <= 2; stallrecovery_phase++)
	{
		...
		if (update_sdp && dynamical_parameters.fix_ext_param_direction)
		{
			...
			{
                            for(auto &filename : read_nsv_file_list(
                                  dynamical_parameters.new_sdp_path))
                              {
					...
					{
						eminus(std::stoi(directions[1])) = approx_obj.d_objective;
						//compute_delta_lag(d_sdp,*this); 
					}
			...
			if (dynamical_parameters.find_boundary && El::Abs(duality_gap) < 0.9)
			{
				...
				if (update_sdp)
				{
					...
					//else 
					{
						...
						std::cout << "new external_step_plus="; for (int i = 0; i < external_step_plus.Height(); i++) std::cout << external_step_plus(i, 0) << " ";
						std::cout << "\n" << std::flush;
