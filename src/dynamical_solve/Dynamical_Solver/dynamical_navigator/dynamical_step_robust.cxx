  ...
  bool &update_sdp, bool &find_zeros, El::Matrix<El::BigFloat> &external_step) 
{
  ...
  for(int stallrecovery_phase=1; stallrecovery_phase <=2; stallrecovery_phase++)
  {
	  ...
	  if (update_sdp)
	  {
		  ...
		  El::LinearSolve(Hpp_minus_Hmixed, external_step);   // after this line, external_step means d_min_p
		  ...
		  if (dynamical_parameters.find_boundary && El::Abs(duality_gap) < 0.9)
		  {
			  ...
			  //std::cout << "grad_mixed.height=" << grad_mixed.Height() << "  .width=" << grad_mixed.Width() << "\n" << std::flush;
			  std::cout << "grad_mixed="; for (int i = 0; i < grad_mixed.Height(); i++) std::cout << grad_mixed(i, 0) << " ";
			  std::cout << "; grad_mixed.search_direction=" << El::Dot(grad_mixed, search_direction) << "\n" << std::flush;
			  ...
			  if (find_zeros && update_sdp)
			  {
				  ...
				  //else 
				  {
					  ...
					  c = El::Dot(d_min_p, grad_corrected) + (dot(grad_x, internal_dx) + dot(grad_y, internal_dy) + lag) + El::Dot(d_min_p, H_pp_d_min_p) / 2;
		  ...
		  if (mu < dynamical_parameters.update_sdp_threshold_min)
			  update_sdp = true;

		  if (stallrecovery_phase == 2)update_sdp = false;
	  ...
	  */
	  ...
	  std::cout << "max_step.beta=" << El::Max(primal_step_length, dual_step_length)*beta << " max_step=" << El::Max(primal_step_length, dual_step_length) << 
		  ". reset beta to 1e50. " << "\n" << std::flush;
	  beta = El::BigFloat(1e50);
  }
