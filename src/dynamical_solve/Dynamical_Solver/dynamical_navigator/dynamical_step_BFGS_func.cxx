...
El::BigFloat compute_primalobj_gradient_V2(
  ...
  const El::DistMatrix<El::BigFloat> &Q)
{
  ...
  El::BigFloat pobj_grad = d_sdp.objective_const
                           + dot(d_sdp.primal_objective_c, x)
                           + dot(sdp.primal_objective_c, dx);

  return pobj_grad;
}

// test x duplication
El::BigFloat compute_primalobj_gradient_V2_test(
  const Block_Info &block_info, const SDP &sdp, const SDP &d_sdp,
  const Block_Vector &x, const Block_Vector &y,
  const Block_Diagonal_Matrix &schur_complement_cholesky,
  const Block_Matrix &schur_off_diagonal,
  const El::DistMatrix<El::BigFloat> &Q)
{
  ...
  for(size_t block(0); block != x.blocks.size(); ++block)
    {
      ...
      std::cout << "rank=" << El::mpi::Rank() << " xID=" << block_index
                << " cx_local=" << cx_local << "\n";
    }
  if(!x.blocks.empty() && x.blocks.at(0).Grid().Rank() != 0)
    {
  ...
  if(El::mpi::Rank() == 0)
    std::cout << "compute dc.x locally : "
              << d_sdp.objective_const + dot(d_sdp.primal_objective_c, x)
              << "\n";
...
El::BigFloat compute_primalobj_gradient_V3(
  const Block_Info &block_info, const SDP &sdp, const SDP &d_sdp,
  const Block_Vector &x, const Block_Vector &y,
  const Block_Diagonal_Matrix &schur_complement_cholesky,
  const Block_Matrix &schur_off_diagonal,
  const El::DistMatrix<El::BigFloat> &Q)
{
  ...
  El::BigFloat dobj_grad = El::Dot(d_sdp.dual_objective_b, y.blocks.at(0))
                           + El::Dot(sdp.dual_objective_b, dy.blocks.at(0))
                           + d_sdp.objective_const;
...
El::BigFloat
compute_primalobj_gradient(const SDP &dsdp, const Block_Vector &x,
                           const Block_Vector &y, const Block_Info &block_info)
{
  El::BigFloat dby;
  if(!y.blocks.empty())
    {
      dby = dsdp.objective_const
            + El::Dotu(dsdp.dual_objective_b, y.blocks.front());
    }
  ...
  if(El::mpi::Rank() == 0)
    std::cout << "dprimalobj = " << dprimalobj << " dcx = " << dcx
              << " dby = " << dby << " xdBy = " << xdBy << "\n"
              << std::flush;
...
void read_sdp_grid(const Dynamical_Solver_Parameters &dynamical_parameters,
      ...
      for(auto &filename :
          read_nsv_file_list(dynamical_parameters.new_sdp_path))
        {
          ...
          boost::algorithm::split(directions, file_name,
                                  boost::is_any_of("_"));
          SDP new_sdp(filename, block_info, grid, timers), d_sdp(new_sdp);
          ...
          if(dir_str == "plus")
            {
              mixed_hess(block_info, d_sdp, x, y, schur_complement_cholesky,
                         schur_off_diagonal, Q, dynamical_parameters.alpha,
                         H_xp, Delta_xy);
              eplus(dir_index1)
                = approx_obj.d_objective; //+ approx_obj.dd_objective;
                                          //compute_delta_lag(d_sdp, *this);

              //eplus(dir_index1) = approx_obj.d_objective + approx_obj.dd_objective;

              /**/
              grad_withoutlog(dir_index1) = compute_primalobj_gradient_V2(
                block_info, sdp, d_sdp, x, y, schur_complement_cholesky,
                schur_off_diagonal, Q);

              if(El::mpi::Rank() == 0)
                std::cout << "compute_primalobj_gradient_V2 :"
                          << grad_withoutlog(dir_index1) << '\n'
                          << std::flush;

              grad_withoutlog(dir_index1) = compute_primalobj_gradient_V3(
                block_info, sdp, d_sdp, x, y, schur_complement_cholesky,
                schur_off_diagonal, Q);

              /**/
              if(El::mpi::Rank() == 0)
                std::cout << "compute_primalobj_gradient_V3 :"
                          << grad_withoutlog(dir_index1) << '\n'
                          << std::flush;
...
void LA_x1_vec1_plus_x2_vec2(const El::BigFloat &x1,
                             const El::Matrix<El::BigFloat> &vec1,
                             const El::BigFloat &x2,
                             const El::Matrix<El::BigFloat> &vec2,
                             El::Matrix<El::BigFloat> &result)
{
...
bool BFGS_update_hessian(const El::Matrix<El::BigFloat> &grad_p_diff,
                         const El::Matrix<El::BigFloat> &last_it_step,
                         El::Matrix<El::BigFloat> &hess_bfgs,
  ...
  if(BFGS_approximate_Hpp_only)
    {
      if(El::mpi::Rank() == 0)
        std::cout << "Use BFGS to update Hpp only"
                  ...
                  << std::flush;
      ;
      BFGS_update_hessian(hess_bfgs.Width(), grad_p_diff, last_it_step,
                          hess_bfgs_pp);
...
void compute_grad_p_grad_mixed_Hpp_Hmixed(
  const Dynamical_Solver_Parameters &dynamical_parameters,
  const El::Matrix<El::BigFloat> &eplus,
  const El::Matrix<El::BigFloat> &eminus, const El::Matrix<El::BigFloat> &esum,
  const std::vector<std::pair<Block_Vector, Block_Vector>> &H_xp,
  const std::vector<std::pair<Block_Vector, Block_Vector>> &Delta_xy,
  ...
  El::Matrix<El::BigFloat> &grad_corrected, El::Matrix<El::BigFloat> &Lpu,
  El::BigFloat &mu, El::Matrix<El::BigFloat> &hess_pp,
  El::Matrix<El::BigFloat> &hess_mixed, El::Matrix<El::BigFloat> &hess_Exact)
{
  int dim_ext_p = dynamical_parameters.n_external_parameters;

  if(dynamical_parameters.use_exact_hessian)
    {
      external_grad_hessian(eplus, eminus, esum, dynamical_parameters.alpha,
                            grad_p, hess_pp);
    }
  else
    {
      if(dynamical_parameters.gradientWithLogDetX == false)
        {
          ...
          if(El::mpi::Rank() == 0)
            std::cout << "the gradient N_p is computed without mu*log(detX) "
                         "term. grad_p=";
        }
      else
        {
          ...
          if(El::mpi::Rank() == 0)
            {
              std::cout << "the gradient N_p is computed with mu*log(detX) "
                           "term. grad_p=";
            }
        }
      print_vector(grad_p);
...
void read_prev_grad_step_hess(
  const Dynamical_Solver_Parameters &dynamical_parameters,
  El::Matrix<El::BigFloat> &prev_grad, El::Matrix<El::BigFloat> &prev_step,
      ...
      for(int i = 0; i < dim_ext_p; i++)
        {
          for(int j = 0; j < dim_ext_p; j++)
            {
              prev_BFGS(i, j)
                = dynamical_parameters.hess_BFGS[i * dim_ext_p + j];
            }
        }
    }
      ...
      for(int j = 0; j < dim_ext_p; j++)
        {
          if(BFGS_approximate_Hpp_only)
            prev_BFGS_pp(i, j)
              = dynamical_parameters.hess_BFGS_pp[i * dim_ext_p + j];
          else
            prev_BFGS_pp(i, j) = 0;
        }
    }
...
void save_load_beta_scan_best_state(
  Block_Vector &dx, Block_Vector &dy, Block_Diagonal_Matrix &dX,
  Block_Diagonal_Matrix &dY, El::BigFloat &primal_step_length,
  El::BigFloat &dual_step_length, El::BigFloat &beta,
  El::Matrix<El::BigFloat> &hess_BFGS, El::Matrix<El::BigFloat> &grad_mixed,

  Block_Vector &dx_best, Block_Vector &dy_best, Block_Diagonal_Matrix &dX_best,
  Block_Diagonal_Matrix &dY_best, El::BigFloat &primal_step_length_best,
  El::BigFloat &dual_step_length_best, El::BigFloat &beta_best,
  El::Matrix<El::BigFloat> &hess_BFGS_best,
  El::Matrix<El::BigFloat> &grad_mixed_best)
{
  dX = dX_best;
  dY = dY_best;
  dx = dx_best;
...
void extrapolate_gradient_as_target_mu(
  ...
  const El::Matrix<El::BigFloat> &grad_p,
  const El::Matrix<El::BigFloat> &grad_mixed_best,
  El::Matrix<El::BigFloat> &grad_BFGS)
{
...
void Dynamical_Solver::compute_dXdY(
  ...
  Block_Vector &dx, Block_Vector &dy, Block_Diagonal_Matrix &dX,
  Block_Diagonal_Matrix &dY, Block_Diagonal_Matrix &R,
  ...
  El::BigFloat &step_length_reduction)
{
  ...
  primal_step_length = step_length(X_cholesky, dX, step_length_reduction,
                                   "run.step.stepLength(XCholesky)", timers);

  dual_step_length = step_length(Y_cholesky, dY, step_length_reduction,
...
void Dynamical_Solver::compute_external_dxdy(
  ...
  Block_Vector &internal_dx, Block_Vector &internal_dy, Block_Vector &dx,
  Block_Vector &dy, Block_Diagonal_Matrix &R,
  ...
  for(int i = 0; i < dynamical_parameters.n_external_parameters; i++)
    {
      for(size_t block = 0; block < x.blocks.size(); ++block)
        {
          El::Axpy(external_step(i), Delta_xy.at(i).first.blocks[block],
                   dx.blocks[block]);
        }
      for(size_t block = 0; block < dy.blocks.size(); ++block)
        {
          El::Axpy(external_step(i), Delta_xy.at(i).second.blocks[block],
                   dy.blocks[block]);
        }
    }
}
...
void Dynamical_Solver::compute_external_dxdydXdY(
  ...
  Block_Vector &internal_dx, Block_Vector &internal_dy, Block_Vector &dx,
  Block_Vector &dy, Block_Diagonal_Matrix &dX, Block_Diagonal_Matrix &dY,
  Block_Diagonal_Matrix &R,
  ...
  El::BigFloat &step_length_reduction)
{
  compute_external_dxdy(dynamical_parameters, internal_dx, internal_dy, dx, dy,
                        R, external_step, Delta_xy);

  compute_dXdY(is_primal_and_dual_feasible, block_info, sdp, grid, X_cholesky,
               Y_cholesky, timers, dx, dy, dX, dY, R, primal_step_length,
               dual_step_length, step_length_reduction);
}
  ...
  Timers &timers)
{
  ...
  compute_corrector_R(block_info, total_psd_rows, x_const, dx_const, y_const,
                      dy_const, X_const, dX_const, Y_const, dY_const, 1, 1, R,
                      R_error, mu, timers);
}

void Dynamical_Solver::internal_step_corrector_iteration_centering(
  const Dynamical_Solver_Parameters &dynamical_parameters,
  const std::size_t &total_psd_rows, const Block_Info &block_info,
  const SDP &sdp, const El::Grid &grid,
  const Block_Diagonal_Matrix &X_cholesky,
  const Block_Diagonal_Matrix &Y_cholesky, Timers &timers,
  ...
  Block_Vector &dx, Block_Vector &dy, Block_Diagonal_Matrix &dX,
  Block_Diagonal_Matrix &dY, Block_Diagonal_Matrix &R, Block_Vector &grad_x,
  Block_Vector &grad_y,
  ...
  El::BigFloat &dual_step_length, El::BigFloat &step_length_reduction)
{
  ...
  internal_predictor_direction_dxdydXdY(
    block_info, sdp, *this, schur_complement_cholesky, schur_off_diagonal,
    X_cholesky, coit_beta, mu, primal_residue_p, Q, grad_x, grad_y, dx, dy, dX,
    dY, R);

  compute_corrector_errors(block_info, sdp, total_psd_rows, x, dx, y, dy, X,
                           dX, Y, dY, 1, 1, error_P, error_p, error_d, error_R,
                           coit_mu, timers);
  ...
  while(error_R > dynamical_parameters.centeringRThreshold
        && --max_corrector_steps > 0)
    {
      internal_corrector_direction(
        block_info, sdp, *this, schur_complement_cholesky, schur_off_diagonal,
        X_cholesky, coit_beta, mu, primal_residue_p, Q, grad_x, grad_y, dx, dy,
        dX, dY, R);

      compute_dXdY(is_primal_and_dual_feasible, block_info, sdp, grid,
                   X_cholesky, Y_cholesky, timers, dx, dy, dX, dY, R);

      // check corrector iteration result
      compute_corrector_errors(block_info, sdp, total_psd_rows, x, dx, y, dy,
                               X, dX, Y, dY, 1, 1, error_P, error_p, error_d,
                               error_R, coit_mu, timers);
  ...
  compute_corrector_errors(block_info, sdp, total_psd_rows, x, dx_last, y,
                           dy_last, X, dX_last, Y, dY_last, 1, 1, error_P,
                           error_p, error_d, error_R, coit_mu, timers);

  if(El::mpi::Rank() == 0)
    std::cout << "dxyXY_last : R_error = " << error_R << "\n" << std::flush;

  // recomputes for primal_step_length, dual_step_length .   step_length_reduction=0.9
  primal_step_length = step_length(X_cholesky, dX_last, El::BigFloat(0.9),
                                   "run.step.stepLength(XCholesky)", timers);
  dual_step_length = step_length(Y_cholesky, dY_last, El::BigFloat(0.9),
                                 "run.step.stepLength(YCholesky)", timers);
  ...
  execute_step(dx_last, dy_last, dX_last, dY_last, primal_step_length,
               dual_step_length);
  ...
  if(El::mpi::Rank() == 0)
    std::cout << "final R error after corrector iteration : " << R_err << "\n"
              << std::flush;
}
...
void Dynamical_Solver::internal_step(
  ...
  const bool &is_primal_and_dual_feasible, El::BigFloat &beta,
  El::BigFloat &primal_step_length, El::BigFloat &dual_step_length,
  El::BigFloat &step_length_reduction)
{
  ...
  if(beta > 0.9 && beta < 1.1)
    {
      return internal_step_corrector_iteration_centering(
        dynamical_parameters, total_psd_rows, block_info, sdp, grid,
        X_cholesky, Y_cholesky, timers, schur_complement_cholesky,
        schur_off_diagonal, Q, dx, dy, dX, dY, R, grad_x, grad_y,
        primal_residue_p, mu, is_primal_and_dual_feasible, beta,
        primal_step_length, dual_step_length, step_length_reduction);
    }

  internal_predictor_direction_dxdydXdY(
    block_info, sdp, *this, schur_complement_cholesky, schur_off_diagonal,
    X_cholesky, beta, mu, primal_residue_p, Q, grad_x, grad_y, dx, dy, dX, dY,
    R);

  internal_corrector_direction(block_info, sdp, *this,
                               schur_complement_cholesky, schur_off_diagonal,
                               X_cholesky, beta, mu, primal_residue_p, Q,
                               grad_x, grad_y, dx, dy, dX, dY, R);
...
void Dynamical_Solver::execute_step(Block_Vector &dx, Block_Vector &dy,
...
void Dynamical_Solver::strategy_hess_BFGS(
  const Dynamical_Solver_Parameters &dynamical_parameters,
  int n_external_parameters, bool lowest_mu_Q,
  El::Matrix<El::BigFloat> &grad_p, El::Matrix<El::BigFloat> &grad_mixed,
  El::Matrix<El::BigFloat> &grad_corrected, El::Matrix<El::BigFloat> &Lpu,
  El::BigFloat &mu, El::Matrix<El::BigFloat> &hess_pp,
  El::Matrix<El::BigFloat> &hess_mixed, El::Matrix<El::BigFloat> &hess_Exact,
  ...
  El::Matrix<El::BigFloat> &hess_BFGS_lowest_mu)
{
  if(dynamical_parameters.use_exact_hessian)
  ...
  else
    {
      ...
      if(dynamical_parameters
           .use_Hmixed_for_BFGS) // I will let simpleboot control whether hess_mixed will be used for hess_BFGS
        {
      ...
      if(lowest_mu_Q == false)
        hess_BFGS = hess_BFGS_lowest_mu;
      ...
      if(dynamical_parameters.prev_grad_step_validQ && lowest_mu_Q)
        {
          ...
          if(dynamical_parameters.BFGS_partial_update_reduction > 0)
            {
              bool exact_update_Q = BFGS_partial_update_hessian(
                El::BigFloat(
                  dynamical_parameters.BFGS_partial_update_reduction),
                grad_diff, prev_step, hess_BFGS);
              if(!exact_update_Q)
                if(El::mpi::Rank() == 0)
                  std::cout
                    << "New hessian is non-positive. partial update. \n"
                    << std::flush;
            }
          else
            {
              bool flippedQ = BFGS_update_hessian(
                grad_diff, prev_step, hess_BFGS, hess_BFGS_pp, hess_mixed,
                update_hess_only_positive, BFGS_approximate_Hpp_only);
              if(flippedQ)
                if(update_hess_only_positive)
                  {
                    if(El::mpi::Rank() == 0)
                      std::cout << "New hessian is non-positive. Rollback to "
                                   "prev_hess. \n"
                                << std::flush;
                  }
                else
                  {
                    if(El::mpi::Rank() == 0)
                      std::cout
                        << "New hessian is non-positive (still updated). \n"
                        << std::flush;
                  }
...
El::BigFloat Dynamical_Solver::finite_mu_navigator(
  ...
  const std::size_t &total_psd_rows, const int dim_y, const El::BigFloat &mu,
  ...
  const Dynamical_Solver_Parameters &dynamical_parameters)
{
  ...
  El::BigFloat lag
    = compute_lag(beta_lag * mu, X_cholesky, *this, block_info, mulogdetX);

  El::BigFloat lag_shifted = lag;
  lag_shifted
    -= total_psd_rows * mu * dynamical_parameters.lagrangian_muI_shift;
...
void Dynamical_Solver::strategy_findboundary_extstep(
  ...
  int n_external_parameters,
  const Dynamical_Solver_Parameters &dynamical_parameters, bool lowest_mu_Q,

  El::Matrix<El::BigFloat> &grad_corrected, El::Matrix<El::BigFloat> &grad_p,
...
{
  lag_shifted = finite_mu_navigator(block_info, X_cholesky, total_psd_rows,
                                    dim_y, mu, beta, dynamical_parameters);
  ...
  if(compute_ext_step_only_once == true && external_step_specified_Q == true
  ...
  else
    {
      ...
      compute_ellipse_boundary(hess_BFGS, grad_corrected, lag_shifted,
                               search_direction, external_step_plus,
  ...
  El::Matrix<El::BigFloat> &grad_BFGS)
{
  if(compare_BFGS_gradient_at_same_mu && (!BFGS_approximate_Hpp_only))
    {
      extrapolate_gradient_as_target_mu(primal_step_length, primal_step_length,
                                        grad_p, grad_mixed, grad_BFGS);
      if(El::mpi::Rank() == 0)
        {
          std::cout << "grad_mixed : ";
          print_vector(grad_mixed);
...
void Dynamical_Solver::save_solver_state(
  const Dynamical_Solver_Parameters &dynamical_parameters,
  const Block_Info &block_info, Block_Vector &dx, Block_Vector &dy,
  Block_Diagonal_Matrix &dX, Block_Diagonal_Matrix &dY,
  const Block_Diagonal_Matrix &schur_complement_cholesky,
  const Block_Matrix &schur_off_diagonal,
  const El::DistMatrix<El::BigFloat> &Q)
{
  ...
  out_folder
    = dynamical_parameters.solver_parameters.checkpoint_out / (".schur");
  if(!std::filesystem::exists(out_folder))
    create_directories(out_folder);
  ...
  save_checkpoint(dynamical_parameters.solver_parameters.checkpoint_out
                    / (".int_ck"),
                  verbosity_save, parameter_properties_save);

  save_checkpoint(dx, dy, dX, dY,
                  dynamical_parameters.solver_parameters.checkpoint_out
                    / (".ext_ck"),
                  verbosity_save, parameter_properties_save);
}
