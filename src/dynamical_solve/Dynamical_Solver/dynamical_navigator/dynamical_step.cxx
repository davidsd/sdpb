  ...
  El::Matrix<El::BigFloat> &external_step)
{
  ...
  beta = predictor_centering_parameter(dynamical_parameters.solver_parameters,
                                       is_primal_and_dual_feasible);
  if(mu_direction_mode == 1)
    beta = 1;
  if(mu_direction_mode == 2)
    beta = 1;
  ...
  internal_predictor_direction(block_info, sdp, *this,
                               schur_complement_cholesky, schur_off_diagonal,
                               X_cholesky, beta, mu, primal_residue_p, Q,
                               grad_x, grad_y, internal_dx, internal_dy, R);
  ...
  if(update_sdp)
    {
              ...
              if(directions[0] == "plus")
                {
                  mixed_hess(block_info, d_sdp, x, y,
                             ...
                             dynamical_parameters.alpha, H_xp, Delta_xy);
                  eplus(std::stoi(directions[1]))
      ...
      if(dynamical_parameters.find_boundary && El::Abs(duality_gap) < 0.9
         && dynamical_parameters.total_iterations > 0)
        {
          El::BigFloat lag = compute_lag(0, X_cholesky, *this);
          std::cout << '\n'
                    << "Lagrangian mu=0: " << lag
                    << ", mu: " << compute_lag(beta * mu, X_cholesky, *this)
                    << "dual_obj: " << dual_objective
                    << ", const: " << sdp.objective_const << '\n'
                    << std::flush;
          ...
          std::cout << '\n'
                    << "search_direction: " << search_direction(0, 0) << ", "
          ...
          if(El::Dot(grad_mixed, search_direction) < 0)
            {
              ...
              //external_step_size = El::Nrm2(external_step);
              external_step *= El::Min(
                El::BigFloat(10), dynamical_parameters.update_sdp_threshold_max
                                    / external_step_size);
              external_step_size = El::Nrm2(external_step);
            }
          if(find_zeros && update_sdp)
            {
              if(El::Dot(grad_mixed, search_direction) < 0)
              ...
              else
                {
                  ...
                  f = +El::Dot(external_step, grad_mixed)
                      + (dot(grad_x, internal_dx) + dot(grad_y, internal_dy)
                         + lag);
  ...
  if(El::Max(primal_step_length, dual_step_length) > 0
     && El::Max(primal_step_length, dual_step_length) < 0.1)
    mu_direction_mode = 2;
  else if(El::Max(primal_step_length, dual_step_length) > 0
          && El::Max(primal_step_length, dual_step_length) < 0.3)
    mu_direction_mode = 1;
  else
    mu_direction_mode = 0;
}
