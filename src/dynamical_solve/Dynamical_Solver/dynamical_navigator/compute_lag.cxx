...
El::Base<El::BigFloat> det_log_cholesky(const Block_Diagonal_Matrix &L)
{
  ...
  for(size_t b = 0; b < L.blocks.size(); b++)
    {
      El::SafeProduct<El::Base<El::BigFloat>> safeDet
        = El::hpd_det::AfterCholesky(El::UpperOrLowerNS::LOWER, L.blocks[b]);
      local_det_log += safeDet.kappa * safeDet.n;

      std::cout << "rank=" << El::mpi::Rank() << " b=" << b
  ...
  El::Base<El::BigFloat> final_result
    = El::mpi::AllReduce(local_det_log, El::mpi::SUM, El::mpi::COMM_WORLD);
...
El::Base<El::BigFloat> det_log_cholesky_fix(const Block_Diagonal_Matrix &L,
      ...
      for(size_t psd_block(0); psd_block < 2; ++psd_block)
        {
          ...
          if(L.blocks.at(blockID).Height() != 0)
            {
              El::SafeProduct<El::Base<El::BigFloat>> safeDet
                = El::hpd_det::AfterCholesky(El::UpperOrLowerNS::LOWER,
                                             L.blocks.at(blockID));
              local_det_log += safeDet.kappa * safeDet.n;
...
compute_delta_lag(const SDP &d_sdp, const Dynamical_Solver &solver)
{
  ...
  for(size_t block(0); block != solver.x.blocks.size(); ++block)
    {
      // dc.x
      local_diff += Dotu(d_sdp.primal_objective_c.blocks.at(block),
                         solver.x.blocks.at(block));
      // temp = dB.y
            ...
            El::BigFloat &mulogdetX)
{
  ...
  El::BigFloat mulogdetX_actual
    = det_log_cholesky_fix(X_cholesky, solver, block_info);
  ...
  for(size_t block(0); block != solver.x.blocks.size(); ++block)
    {
      ...
      local_residues += Dotu(solver.dual_residues.blocks.at(block),
                             solver.x.blocks.at(block));
    }

  El::BigFloat dual_residue_dot_x
    = El::mpi::AllReduce(local_residues, El::mpi::SUM, El::mpi::COMM_WORLD);

  lag += dual_residue_dot_x;
