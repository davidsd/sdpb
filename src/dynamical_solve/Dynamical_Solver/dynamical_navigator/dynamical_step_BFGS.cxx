...
void external_window(const std::vector<El::BigFloat> &bmax,
                     ...
                     const int n_parameters, El::Matrix<El::BigFloat> &step)
{
  El::Print(step);
  for(int i = 0; i < n_parameters; i++)
    {
      step(i) = El::Min(El::Max(bmin.at(i) - external_coord.at(i), step(i)),
                        bmax.at(i) - external_coord.at(i));
    }
  El::Print(step);
}
...
template <typename T> void print_stdvector(const std::vector<T> &vec)
{
  if(El::mpi::Rank() == 0)
    {
...
template <typename T> void print_stdpair(const std::vector<T> &vec)
{
  if(El::mpi::Rank() == 0)
    {
      for(auto it = vec.begin(); it != vec.end(); it++)
...
void read_sdp_grid(
  ...
  El::Matrix<El::BigFloat> &eplus, El::Matrix<El::BigFloat> &eminus,
...
{
  bool
  operator()(const std::pair<T1, T2> &left, const std::pair<T1, T2> &right)
  {
    Pred p;
...
bool CubicApprox_LineSearch_RecommendPoint(
  std::vector<std::pair<El::BigFloat, El::BigFloat>> &history,
  const El::BigFloat &beta_scan_end, const El::BigFloat &beta_scan_step,
  El::BigFloat &new_x);
...
bool compute_ext_step_only_once = true;
bool recompute_ext_during_re_aiming
  = true; // if true, ext-step will be recomputed during re-aiming at the lowest mu, even if compute_ext_step_only_once = true
...
El::BigFloat
predictor_centering_parameter_V3(const Solver_Parameters &parameters,
                                 const El::BigFloat &mu,
                                 const El::BigFloat &mu_near_optimal_threshold)
{
  return mu < mu_near_optimal_threshold
           ? parameters.feasible_centering_parameter
           : parameters.infeasible_centering_parameter;
}

El::BigFloat compute_beta_for_finite_dGap(const El::BigFloat &current_dGap,
                                          const El::BigFloat &dGap_target)
{
  El::BigFloat beta = dGap_target / current_dGap;
  if(beta < 0.1)
    beta = El::BigFloat(0.1);
  return beta;
}
  ...
  El::Matrix<int32_t> &block_timings_ms)
{
  ...
  if(!(dynamical_parameters.external_corrector_Q
       && external_corrector_jump_to_newSDP))
    {
  ...
  El::BigFloat beta_internal = predictor_centering_parameter_V3(
    dynamical_parameters.solver_parameters, dGap_current,
    dgap_near_optimal_threshold);
  if(beta_scan_begin_El < 0)
    beta_scan_begin_El = beta_internal;
  ...
  if((dGap_current > 0 && dynamical_parameters.dualityGap_upper_limit > 0
      && dGap_current > dynamical_parameters.dualityGap_upper_limit)
     || duality_gap == El::BigFloat(0))
    {
      ...
      return internal_step(
        ...
        primal_residue_p, mu, is_primal_and_dual_feasible, beta_internal,
  ...
  if(dynamical_parameters.external_corrector_Q
     && external_corrector_jump_to_newSDP)
    {
      ...
      return external_corrector_run(
        dynamical_parameters, sdp, block_info, grid, dx, dy, dX, dY,
        primal_step_length, dual_step_length, step_length_reduction, timers);
    }
  ...
  if(El::mpi::Rank() == 0)
    std::cout
      ...
      << " abs(current-target)="
      << El::Abs(dynamical_parameters.finite_dGap_target - dGap_current)
      << " dualityGap_upper_limit="
      << dynamical_parameters.dualityGap_upper_limit << " bool1="
      << (El::Abs(dynamical_parameters.finite_dGap_target - dGap_current)
          > dynamical_parameters.centeringRThreshold)
      << " bool2="
      << (dynamical_parameters.centeringRThreshold > 0
          && R_error > dynamical_parameters.centeringRThreshold)
      << "\n";
  ...
  if(dynamical_parameters.finite_dGap_target > 0
     && (El::Abs(dynamical_parameters.finite_dGap_target - dGap_current)
         ...
         || (dynamical_parameters.centeringRThreshold > 0
             && R_error > dynamical_parameters.centeringRThreshold)))
    {
      beta = compute_beta_for_finite_dGap(
        dGap_current, dynamical_parameters.finite_dGap_target);
      update_sdp = false;
      if(El::Min(p_step, d_step) < 0.1)
        beta = El::Min(1 / (El::Min(p_step, d_step) * 10), El::BigFloat(1e10));

      if(El::mpi::Rank() == 0)
        std::cout << "run centering steps with beta=" << beta
                  << " duality Gap target="
                  << dynamical_parameters.finite_dGap_target
                  << " abs(target-current)="
                  << El::Abs(dynamical_parameters.finite_dGap_target
                             - dGap_current)
  ...
  if(dynamical_parameters.finite_dGap_target > 0
     && dynamical_parameters.returnCheckpointOnLCP
     && dynamical_parameters.centeringRThreshold > 0
     && R_error < dynamical_parameters.centeringRThreshold
     && El::Abs(dynamical_parameters.finite_dGap_target - dGap_current)
          < dynamical_parameters.centeringRThreshold)
  ...
  if(dynamical_parameters.centeringRThreshold > 0
     && R_error > dynamical_parameters.centeringRThreshold)
    {
      if(El::mpi::Rank() == 0)
        std::cout << "run centering steps with beta=1 \n";
      beta = 1;
      update_sdp = false;
  ...
  El::Matrix<El::BigFloat> prev_grad(n_external_parameters, 1),
    prev_step(n_external_parameters, 1),
    prev_BFGS(n_external_parameters, n_external_parameters),
    prev_BFGS_pp(n_external_parameters, n_external_parameters);
  El::Zeros(hess_Exact, n_external_parameters, n_external_parameters);
  El::Zeros(grad_mixed, n_external_parameters, 1);

  read_prev_grad_step_hess(dynamical_parameters, prev_grad, prev_step,
                           prev_BFGS, prev_BFGS_pp);
  ...
  // beta scan
  for(beta = beta_scan_begin_El; beta <= beta_scan_end_El;
      beta += beta_scan_step_El)
    {
      ...
      compute_grad_p_grad_mixed_Hpp_Hmixed(
        dynamical_parameters, eplus, eminus, esum, H_xp, Delta_xy, internal_dx,
        internal_dy, grad_withoutlog, grad_withlog, grad_p, grad_mixed,
        grad_corrected, Lpu, mu, hess_pp, hess_mixed, hess_Exact);
      ...
      strategy_hess_BFGS(
        dynamical_parameters, n_external_parameters, lowest_mu_Q, grad_p,
        grad_mixed, grad_corrected, Lpu, mu, hess_pp, hess_mixed, hess_Exact,
        prev_BFGS, prev_BFGS_pp, prev_step, prev_grad, hess_BFGS_lowest_mu);

      // compute external step
      if(dynamical_parameters.find_boundary)
        {
          strategy_findboundary_extstep(
            block_info, X_cholesky, total_psd_rows, y.size(), mu, beta,
            n_external_parameters, dynamical_parameters, lowest_mu_Q,
            grad_corrected, grad_p, grad_mixed, external_step,
            external_step_save, external_step_specified_Q);
        }
      else
        {
          findMinimumQ = true;
          ...
          lag_shifted
            = finite_mu_navigator(block_info, X_cholesky, total_psd_rows,
                                  y.size(), mu, beta, dynamical_parameters);
      ...
      compute_external_dxdydXdY(
        is_primal_and_dual_feasible, dynamical_parameters, block_info, sdp,
        grid, X_cholesky, Y_cholesky, timers, internal_dx, internal_dy, dx, dy,
        dX, dY, R, external_step, Delta_xy, primal_step_length,
      ...
      if(El::mpi::Rank() == 0)
        {
          ...
          std::cout << "P/D-step-len-ext = " << primal_step_length << " , "
                    << dual_step_length << "\n"
                    << std::flush;
      ...
      if(beta == beta_scan_begin_El
         || El::Max(primal_step_length, dual_step_length)
              > El::Max(primal_step_length_best, dual_step_length_best))
        {
          // save best state
          save_load_beta_scan_best_state(
            dx_best, dy_best, dX_best, dY_best, primal_step_length_best,
            dual_step_length_best, beta_best, hess_BFGS_best, grad_mixed_best,
            dx, dy, dX, dY, primal_step_length, dual_step_length, beta,
            hess_BFGS, grad_mixed);
        }
      ...
      if(El::Max(primal_step_length, dual_step_length)
         > dynamical_parameters.step_max_threshold)
        break;
    }

  // load best state
  save_load_beta_scan_best_state(
    dx, dy, dX, dY, primal_step_length, dual_step_length, beta, hess_BFGS,
    grad_mixed, dx_best, dy_best, dX_best, dY_best, primal_step_length_best,
    dual_step_length_best, beta_best, hess_BFGS_best, grad_mixed_best);

  if(El::mpi::Rank() == 0)
    std::cout << "best scan result : best beta = " << beta << " step-len : ("
              << primal_step_length << ", " << dual_step_length << ")\n"
              << std::flush;

  if(El::mpi::Rank() == 0)
    std::cout << "dualityGap_upper_limit="
              << dynamical_parameters.dualityGap_upper_limit
              << " prev_grad_step_validQ="
              << dynamical_parameters.prev_grad_step_validQ
              << " use_Hmixed_for_BFGS="
              << dynamical_parameters.use_Hmixed_for_BFGS
              << " beta_for_mu_logdetX="
              << dynamical_parameters.beta_for_mu_logdetX << "\n"
              << std::flush;

  // check beta scan result
  if(El::Max(primal_step_length, dual_step_length)
       > dynamical_parameters.step_min_threshold
     || max_climbing <= 0 || dynamical_parameters.finite_dGap_target > 0
     || (dynamical_parameters.dualityGap_upper_limit > 0
         && 2 * mu * total_psd_rows
              > dynamical_parameters.dualityGap_upper_limit))
    {
      // hopping step
      ...
      compute_corrector_R(block_info, total_psd_rows, x, dx, y, dy, X, dX, Y,
                          dY, 1, 1, R_0, R_err, coit_mu, timers);
      compute_R_error(total_psd_rows, X, Y, R_err2, coit_mu2, timers);
      if(El::mpi::Rank() == 0)
        std::cout << "before hopping step, R_err(X)=" << R_err2
                  << " mu=" << coit_mu2 << " R_err(X+dX)=" << R_err
                  << " mu=" << coit_mu << "\n"
                  << std::flush;
      ...
      if(El::mpi::Rank() == 0)
        std::cout << "after hopping step, R_err = " << R_err
                  << " mu=" << coit_mu << "\n"
                  << std::flush;

      external_step *= dual_step_length;
      external_step_size = El::Nrm2(external_step);
      strategy_update_grad_BFGS(primal_step_length, dual_step_length, grad_p,
                                grad_mixed, grad_BFGS);
      return;
    }
  ...
  return internal_step(dynamical_parameters, total_psd_rows, block_info, sdp,
                       grid, X_cholesky, Y_cholesky, timers,
                       schur_complement_cholesky, schur_off_diagonal, Q, dx,
                       dy, dX, dY, R, grad_x, grad_y, primal_residue_p, mu,
                       is_primal_and_dual_feasible, beta, primal_step_length,
                       dual_step_length, step_length_reduction);
}
