...
Dynamical_Solver::Dynamical_Solver(
  const Dynamical_Solver_Parameters &dynamical_parameters,
  const Verbosity &verbosity, const bool &require_initial_checkpoint,
...
{
  if(!load_checkpoint(dynamical_parameters.solver_parameters.checkpoint_in,
                      block_info, verbosity, require_initial_checkpoint))
    {
      X.set_zero();
      Y.set_zero();
      ...
      // X = \Omega_p I
      X.add_diagonal(
        dynamical_parameters.solver_parameters.initial_matrix_scale_primal);
      // Y = \Omega_d I
      Y.add_diagonal(
        dynamical_parameters.solver_parameters.initial_matrix_scale_dual);
    }
