...
inline void addScaledVector(Vector &v, const Real &a, const Vector &u)
{
  ...
  for(unsigned int i = 0; i < v.size(); i++) v[i] += a * u[i];
}
...
{
  Real result = 0;
  for(unsigned int i = 0; i < u.size(); i++) result += u[i] * v[i];
  return result;
}
...
{
  Vector w(u.size());
  for(unsigned int i = 0; i < w.size(); i++) w[i] = u[i] * v[i];
  return w;
}
